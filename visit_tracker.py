...
def _normalize(name: str) -> str:
    ...
    name = re.sub(r'הזמנה[\s\-:]*\d+', '', name)
    name = re.sub(r'מספר הזמנה\s*:?\s*\d*', '', name)
    name = re.sub(r':מספר הזמנה\s*\d*', '', name)
    name = re.sub(r'\s+\d{6,}\s*:?\s*מספר.*$', '', name)
    name = re.sub(r'\s+הזמנה\b.*$', '', name)
